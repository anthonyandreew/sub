...
def read_provider(path: Path) -> tuple[list[str], list[str]]:
    chunks = re.split(r"(?=^  - name: )", path.read_text(encoding="utf-8"), flags=re.MULTILINE)
...
def node_priority(node: str) -> int:
    """Put the simple, Android-compatible Hysteria2 node first."""
...
def with_benchmark(node: str) -> str:
    """Use an HTTP 204 endpoint, as recommended for Stash delay tests."""
...
def main() -> None:
    ...
    nodes, names = zip(*sorted(zip(nodes, names), key=lambda pair: node_priority(pair[0])))
    ...
    content = """#SUBSCRIBED https://raw.githubusercontent.com/anthonyandreew/sub/main/clash_config_android.yaml
...
""" + "".join(nodes)
    ...
    rules = [
        ...
        *domain_rules, *ipv4_rules, *ipv6_rules, "  - MATCH,PROXY",
    ]
